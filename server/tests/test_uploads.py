...
def create_large_image(size_mb=6):
    """Create a large image file (exceeds 5MB limit)."""
    # Create a large image by making it very wide/tall
    pixels = size_mb * 1024 * 1024 // 3  # Approximate pixels for RGB
    width = int(pixels ** 0.5)
    height = width
    return create_test_image(size=(width, height))
