...
class TestEmailVerificationTokenUtility:
    ...
    def test_verify_email_token_success(self, app, create_user):
        ...
        with app.app_context():
            user_id = create_user(email="verify@example.com", username="verifyuser", email_verified=False)
            user = User.query.get(user_id)
            raw_token = create_email_verification(user)
            db.session.commit()
...
class TestResendVerificationEndpoint:
    ...
    def test_resend_verification_authenticated(self, client, app, create_unverified_user, mock_send_verification_email):
        """Logged-in unverified user can resend."""
        with app.app_context():
            user_id = create_unverified_user(
                username="resenduser",
    ...
    def test_resend_verification_by_email(self, client, app, create_unverified_user, mock_send_verification_email):
        ...
        response = client.post("/api/auth/resend-verification", json={"email": "byemail@example.com"})
        assert response.status_code == 200
        mock_send_verification_email.assert_called_once()
        assert mock_send_verification_email.call_args[0][0] == "byemail@example.com"

    def test_resend_verification_already_verified(self, client, create_user, mock_send_verification_email):
...
class TestRegisterSendsVerification:
    ...
    def test_register_returns_verification_required(self, client, sample_user_data, mock_send_verification_email):
