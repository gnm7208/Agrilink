...
- The PWA layer is three files in `client/public/` — `manifest.webmanifest`, `sw.js`, and the icon set (`favicon.svg`, `icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`) — plus `client/src/services/register-sw.js`, which registers the worker only when `import.meta.env.PROD` is true.
...
- `client/public/.well-known/assetlinks.json` must contain the SHA-256 of the key in `~/.android-signing/agrilink.keystore` and the package id `com.gnm7208.agrilink`. If the key ever changes, regenerate this file or the Android app falls back to a browser tab with a URL bar.
- `client/public/privacy.html` is the store-facing privacy policy; keep it truthful when new data is collected.
- The Android (TWA) project is generated by Bubblewrap from `../store-packaging/agrilink/twa-manifest.json`; rebuild with `bubblewrap update && bubblewrap build` there after a manifest/icon change, and bump `appVersionCode` for every store upload.
